...
void inicia_jogo(void) {
	...
	printf("Regras:\nDado um tabuleiro 3x3 com 8 peças (cada peça tem um número de 1 a 8) e um espaço vazio.\n O objetivo é colocar os números nas peças em ordem de forma com a que fique igual configuração final usando o espaço vazio. \nPodemos deslizar quatro peças adjacentes (esquerda, direita, acima e abaixo) no espaço vazio.");
	printf("\nO espaço vazio é representado como 0 para facilitar a visualização.");

	printf("\nConfiguração final:\n");
	printf("\n\t1\t2\t3\n\t4\t5\t6\n\t7\t8\t0\n\n");

	printf("Deseja começar o jogo: S/N ");
	scanf("%c", &resposta);
	...
	if (resposta == 'S' || resposta == 's') {
		...
		printf("\n!!!PARABENS VOCÊ VENCEU O JOGO DOS OITO!!!");
	}
